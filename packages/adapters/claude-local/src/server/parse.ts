...
export function isClaudeThinkingBlockReplayError(parsed: Record<string, unknown>): boolean {
  const resultText = asString(parsed.result, "").trim();
  const allMessages = [resultText, ...extractClaudeErrorMessages(parsed)]
    .map((msg) => msg.trim())
    .filter(Boolean);
  ...
  return allMessages.some((msg) =>
    /(?:thinking|redacted_thinking).*cannot be modified|thinking.*must remain as they were/i.test(msg),
  );
}
...
export function isClaudeTransientUpstreamError(input: {
  ...
  if (parsed && (isClaudeMaxTurnsResult(parsed) || isClaudeUnknownSessionError(parsed) || isClaudePoisonedPreviousMessageIdError(parsed) || isClaudeImageProcessingError(parsed) || isClaudeThinkingBlockReplayError(parsed))) {
    return false;
  }
