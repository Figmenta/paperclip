...
A comment authored by a board API key (`actorSource == "board_key"`) is a system actor, not an interactive human, even though authorization normalizes it to `actorType == "user"`. A board-key comment that carries no explicit `reopen`/`resume` intent must be inert on a terminal (`done`/`cancelled`) issue: it is retained as issue-thread activity but must not implicitly reopen the issue or wake the finished assignee. This is the cross-service result-mirror contract — a system service (for example Orchestra posting a deliverable onto the just-closed mirror after its own task went terminal) records the deliverable without re-running completed work. A board-key caller that genuinely intends to reopen must pass explicit `reopen`/`resume`, which flows through the normal explicit-reopen path. This carve-out does not weaken human-comment semantics: interactive users comment as `session` (or local `local_implicit`) and retain the implicit-reopen behavior above.
