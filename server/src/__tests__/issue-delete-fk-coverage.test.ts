 ...
 *
 * `restrict` is deliberately NOT in DB_RESOLVED_ACTIONS: Postgres blocks the delete on it exactly
 * as it does on `no action`, so it must reach `remove()`. A RESTRICT + notNull column admits
 * neither treatment above — it cannot be detached, and hard-deleting it may destroy a record
 * upstream only ever soft-deletes — so the third disposition is to refuse the delete with a typed
 * 422. `company_skill_test_runs.issue_id` is the first of that kind (FIG-785).
 */
...
const HANDLED_BY_REMOVE = [
  ...
  // refused, or deleted only when already soft-deleted — RESTRICT + notNull, so it can be
  // neither detached nor unconditionally deleted. A live run makes remove() throw 422; a run
  // whose deletedAt is set is hard-deleted, that being the one state in which upstream itself
  // discards the row. See the FIG-785 block in services/issues.ts remove().
  "company_skill_test_runs.issue_id",
];
